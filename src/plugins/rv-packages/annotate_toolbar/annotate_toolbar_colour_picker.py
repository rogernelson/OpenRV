...
class ColourPickerPopup(QtWidgets.QWidget):
    """Floating HSV colour picker popup.

    Show via show_at(global_pos). Emits colour_changed(QColor) on every
    change. Dismisses automatically when the user clicks outside (Qt.Popup).
    """
    ...
    def _build_ui(self):
        # Outer layout holds a single styled card widget
        outer = QtWidgets.QVBoxLayout(self)
        outer.setContentsMargins(4, 4, 4, 4)
        outer.setSpacing(0)

        card = QtWidgets.QFrame()
        card.setObjectName("ColourPickerCard")
        card.setStyleSheet("""
            QFrame#ColourPickerCard {
                background: #252525;
    ...
    def show_at(self, global_pos):
        """Position popup below global_pos and show it."""
